...
namespace server
{
    ...
    bool Server::process_packet(ENetPeer *peer, ENetPacket *packet)
    {
        ...
        case player::NET_MESSAGE_GENERIC_TEXT:
        {
            ...
            if (m_config->m_server.platformID == "4")
            {
                text_parse.remove("fz");
                text_parse.remove("totalPlaytime");
                text_parse.remove("klv");
                text_parse.remove("hash2");
                text_parse.remove("zf");
                text_parse.add("gid", gid);
                text_parse.add("tr", "4322");
            }
