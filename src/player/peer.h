...
public:
    ...
    int send_raw_packet(eNetMessageType type, GameUpdatePacket* game_update_packet,
        std::size_t length = sizeof(GameUpdatePacket), uint8_t* extended_data = nullptr,
        enet_uint32 flags = ENET_PACKET_FLAG_RELIABLE);

    int
    send_variant(VariantList&& variant_list, uint32_t net_id = -1, enet_uint32 flags = ENET_PACKET_FLAG_RELIABLE);
