...
namespace client {
Client::Client(Config* config, server::Server* server)
    : enetwrapper::ENetClient{}, m_config{ config }, m_peer{ nullptr }, m_server{ server },
      m_redirecting{ false } {}
...
bool Client::process_tank_update_packet(ENetPeer* peer, player::GameUpdatePacket* game_update_packet)
{
    if (game_update_packet->type != player::PACKET_STATE
        && game_update_packet->type != player::PACKET_CALL_FUNCTION) {
    ...
    case player::PACKET_CALL_FUNCTION: {
        ...
        case "OnSendToServer"_fh: {
            ...
            m_server->get_peer()->send_variant(
                { "OnSendToServer", m_config->m_host.port, variant_list.Get(2).GetINT32(),
                  variant_list.Get(3).GetINT32(),
                  fmt::format("127.0.0.1|{}|{}", tokenize.size() == 2 ? "" : tokenize.at(1),
        ...
        case "OnSpawn"_fh: {
            ...
            if (text_parse.get("type", 1) == "local") {
                ...
                m_server->get_peer()->send_raw_packet(player::NET_MESSAGE_GAME_PACKET, game_update_packet,
